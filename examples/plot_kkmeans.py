...
def plot_2D_scatter(axs):
    ...
    axs.scatter(X[:, 0], X[:, 1], c=cluster_assignments, cmap='viridis', marker='o')
...
def plot_5D_reduced_scatter(axs):
    ...
    axs.scatter(X_pca[:, 0], X_pca[:, 1], c=cluster_assignments, cmap='viridis', marker='o')
    axs.set_title('High dimensional data-set reduced to 2D using PCA)', fontsize=FONT_SIZE)

def plot_1D_scatter(axs):
    X, _= make_blobs(n_samples=300, centers=3, n_features=1, random_state=42)
    cluster_assignments = kkmeans(X, n_clusters=3)

    axs.scatter(X[:, 0], np.zeros_like(X), c=cluster_assignments, cmap='viridis', marker='o')
...
def plot_large_data_set(axs):
    ...
    axs.scatter(X[:, 0], X[:, 1], c=cluster_assignments, cmap='viridis', marker='o', s=5, alpha=0.5)
